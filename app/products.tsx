...
import { View, Text, FlatList, TouchableOpacity, Dimensions, TextInput, ActivityIndicator, ScrollView } from 'react-native';
...
export default function AllProductsScreen() {
  ...
  const products = (productsData as any)?.pages.flatMap((page: any) => page.products) || [];
    ...
    return (
      <>
        ...
        <View className="flex-1 bg-background items-center justify-center">
          <Text className="text-lg text-red-500 mb-4">Failed to load products</Text>
          <TouchableOpacity
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              ...
              {subcategories.map((subcategory) => (
                <TouchableOpacity
                  ...
                  onPress={() => setSelectedSubcategory(selectedSubcategory === subcategory.id ? null : subcategory.id)}
                >
                  <Text className={`text-sm font-medium ${
                    selectedSubcategory === subcategory.id ? 'text-white' : 'text-text'
                  }`}>
                    {subcategory.name}
                  </Text>
                </TouchableOpacity>
        ...
        {isLoading ? (
          ...
          <View className="flex-1 justify-center items-center px-4">
            <Text className="text-lg text-textSecondary text-center mb-4">
              {debouncedSearchQuery || selectedCategory ? 'No products found matching your criteria' : 'No products available'}
            </Text>
            {(debouncedSearchQuery || selectedCategory || selectedSubcategory) && (
              <TouchableOpacity
                className="bg-primary px-4 py-2 rounded-lg"
            ...
            ListFooterComponent={
              isFetchingNextPage ? (
                <View className="py-4 items-center">
                  ...
                  <Text className="text-textSecondary mt-2">Loading more...</Text>
                </View>
              ) : null
            }
          />
